...
def runTest(numRuns, numEpisodes, agent, env, parameters, report=False):
    stepsData = []
    ...
    for i in range(numRuns):
        steps, returns = runAgent(numEpisodes, agent, env, parameters, report=report)
        if report:
            print("Run", i)
...
def runAgent(numEpisodes, agentClass, envClass, parameters, report=False):
    # seed np
    ...
    # run test
    for i in range(numEpisodes):
        glue.runEpisode(max_steps=MAX_STEPS)
        if report:
...
def main():
    ...
    data = []
    for agent in agents:
        data.append(runTest(numRuns, numEpisodes, agent, env, parameters, report=True))
